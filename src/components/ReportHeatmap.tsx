        ...
        if (active && payload && payload.length) {
            ...
            return (
                <div className="bg-background border border-border rounded-lg p-3 shadow-lg">
                    ...
                    <p className="text-sm">
                        <span className="font-medium">ระดับความเร่งด่วน:</span> {data.urgency}
                    </p>
                    <p className="text-sm">
                        <span className="font-medium">จำนวนรายงาน:</span> {data.count}
                    </p>
                    <p className="text-sm">
                        <span className="font-medium">ผู้ประสบภัยทั้งหมด:</span> {data.totalPeople} คน
                    </p>
                </div>
            );
        }
                    ...
                    <div className="w-full">
                            ...
                            >
                                ...
                                <Scatter data={heatmapData} shape="circle">
                                    {heatmapData.map((entry, index) => (
                                        <Cell
                                            key={`cell-${index}`}
                                            fill={URGENCY_COLORS[entry.urgency as keyof typeof URGENCY_COLORS]}
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 mt-4 text-xs">
                            {HELP_CATEGORIES.map((cat) => (
                                <div key={cat.id} className="flex items-center gap-1 text-muted-foreground">
                                    <span>{cat.icon}</span>
                                    <span>{cat.label}</span>
                                </div>
                            ))}
                            ...
                            {[1, 2, 3, 4, 5].map((level) => (
                                <div key={level} className="flex items-center gap-2">
                                    <div
                                        ...
                                        style={{ backgroundColor: URGENCY_COLORS[level as keyof typeof URGENCY_COLORS] }}
                                    />
